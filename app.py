...
def plot_confidence_intervals(sample_means, confidence_intervals, population_mean):
    # Use the recommended figure/axes pattern
    fig, ax = plt.subplots(figsize=(10, 6))

    # Convert confidence intervals to error bars format
    error_bars = [
        (mean - ci[1], ci[2] - mean)
        for mean, ci in zip(sample_means, confidence_intervals)
    ]
    ...
    for i, mean in enumerate(sample_means):
        ax.errorbar(
            i,
            mean,
            yerr=[[lower_bounds[i]], [upper_bounds[i]]],
            fmt="o",
        )
    ...
    ax.hlines(
        population_mean,
        xmin=-1,
        xmax=len(sample_means),
        colors="yellow",
        label="Population Mean",
    )

    # Formatting
    ax.set_title("Sampling Distribution and Confidence Intervals")
...
st.write(
    """
    This simulator generates a number of samples from a population with a specified
    mean and standard deviation. It calculates the confidence intervals for the mean
    of each sample and plots them.

    The table below the plot shows the total number of intervals generated and how
    many of those do not contain the population mean.

    You can adjust the population parameters, sample size, number of samples, and
    confidence level using the sidebar controls.
...
for _ in range(int(number_of_samples)):
    sample = np.random.choice(population_data, size=int(sample_size), replace=False)
...
if st.button("Generate Confidence Intervals"):
    ...
    data = {
        "Total Intervals Generated": [int(number_of_samples)],
